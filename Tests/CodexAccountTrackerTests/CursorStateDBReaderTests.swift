...
final class CursorStateDBReaderTests: XCTestCase {
    ...
    func testCursorDiskKVAnchoredLikeExcludesDecoy() throws {
        ...
        XCTAssertEqual(rows.count, 6, "anchored prefix should match exactly the six bubbles")
    ...
    func testComposerMetadataAndBubbleRows() throws {
        ...
        XCTAssertEqual(agentBubbles.count, 6, "colon-keyed bubbles parse the composerId from segment 2")
        XCTAssertEqual(Set(agentBubbles.compactMap(\.modelName)), ["claude-4.5-opus-high-thinking"])
        // Tokens are logged on separate null-model bubbles (raw per-bubble values).
        XCTAssertEqual(agentBubbles.map(\.inputTokens).reduce(0, +), 1_800_000)
        XCTAssertEqual(agentBubbles.map(\.outputTokens).reduce(0, +), 200_000)
        XCTAssertEqual(agentBubbles.map(\.inputTokens).max(), 1_000_000)
        // The context-window signal is read from contextWindowStatusAtCreation.tokensUsed.
        XCTAssertEqual(agentBubbles.map(\.contextTokens).max(), 1_500_000)
